...
const APP_NAME = 'Charger Wallet  Example';
app.name = APP_NAME;
...
function createMainWindow() {
  ...
  browserWindow.webContents.on('did-finish-load', () => {
    ...
    browserWindow.webContents.send(ipcMessageKeys.INJECT_CHARGERWALLET_DESKTOP_GLOBALS, {
  ...
  session.defaultSession.webRequest.onBeforeSendHeaders(filter, (details, callback) => {
    ...
    if (url.startsWith('http://127.0.0.1:21320/') || url.startsWith('http://localhost:21320/')) {
      // resolve charger walletbridge CORS error
      details.requestHeaders.Origin = 'https://jssdk.chargerwallet.com';
    }
