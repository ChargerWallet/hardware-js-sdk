...
export class Device extends EventEmitter {
  ...
  toMessageObject(): DeviceTyped | null {
    ...
    return {
      ...
      bleName,
      name: bleName || label || `Charger Wallet  ${deviceType?.toUpperCase()}`,
  ...
  getMode() {
    if (this.features?.bootloader_mode) {
      // bootloader mode
      return EChargerWalletDeviceMode.bootloader;
    }

    if (!this.features?.initialized) {
      // not initialized
      return EChargerWalletDeviceMode.notInitialized;
    }

    if (this.features?.no_backup) {
      // backup mode
      return EChargerWalletDeviceMode.backupMode;
    }

    // normal mode
    return EChargerWalletDeviceMode.normal;
  }
