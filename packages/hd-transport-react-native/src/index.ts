...
import {
  ...
  Characteristic,
  ScanMode,
} from '@chargerwallet/react-native-ble-plx';
import ByteBuffer from 'bytebuffer';
import transport, { COMMON_HEADER_SIZE, LogBlockCommand } from '@chargerwallet/hd-transport';
import { createDeferred, Deferred, ERRORS, HardwareErrorCode } from '@chargerwallet/hd-shared';
import type EventEmitter from 'events';
import { initializeBleManager, getConnectedDeviceIds, getBondedDevices } from './BleManager';
import { subscribeBleOn } from './subscribeBleOn';
import {
  isChargerWalletDevice,
...
export default class ReactNativeBleTransport {
  ...
  /**
   * 获取设备列表
   * 在搜索超过超时时间或设备数量大于 5 台时，返回 Charger Wallet  设备，
   * @returns
   */
  async enumerate() {
    ...
    return new Promise<Device[]>(async (resolve, reject) => {
      ...
      blePlxManager.startDeviceScan(
        ...
        (error, device) => {
          ...
          if (isChargerWalletDevice(device?.name ?? null, device?.id)) {
